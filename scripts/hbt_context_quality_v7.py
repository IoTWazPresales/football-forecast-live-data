...
CITY_ALIAS={'la coruna':'A Coruña','a coruna':'A Coruña','munchen':'München','malaga':'Málaga'}
...
def http_json(url:str,timeout:int=25,headers:dict[str,str]|None=None)->dict[str,Any]:
...
def geocode_ok(g:Any,iso:str|None)->bool:
    ...
    if iso and not gc:
        acceptable={
            'GB':{'united kingdom','england','scotland','wales','northern ireland'},'ES':{'spain'},'FR':{'france'},
            'DE':{'germany'},'IT':{'italy'},'NL':{'netherlands'},'MC':{'monaco'}
        }.get(iso,set())
        if acceptable and norm(g.get('country')) not in acceptable:return False
    return True

def terrain_elevation(lat:Any,lon:Any)->float|None:
    try:
        q=urllib.parse.urlencode({'latitude':lat,'longitude':lon}); raw=http_json('https://api.open-meteo.com/v1/elevation?'+q)
        z=(raw.get('elevation') or [None])[0]
        return None if z is None else float(z)
    except Exception:return None

def robust_geocode(city:str,country:str)->dict[str,Any]|None:
    if not city:return None
    iso=expected_iso(city,country); raw_city=city.split(',')[0].strip(); query_city=CITY_ALIAS.get(norm(raw_city),raw_city)
    q={'name':query_city,'count':25,'language':'en','format':'json'}
    if iso:q['countryCode']=iso
    raw=http_json('https://geocoding-api.open-meteo.com/v1/search?'+urllib.parse.urlencode(q)); rows=raw.get('results') or []
    if not rows:return None
    c=rows[0]
    if iso and str(c.get('country_code') or '').upper()!=iso:return None
    elev=c.get('elevation')
    try:bad_elev=elev is not None and float(elev)>5000
    except Exception:bad_elev=True
    if elev is None or bad_elev:elev=terrain_elevation(c.get('latitude'),c.get('longitude'))
    return {'lat':c.get('latitude'),'lon':c.get('longitude'),'elevationM':elev,'resolvedName':c.get('name'),'country':c.get('country'),'countryCode':c.get('country_code'),'admin1':c.get('admin1'),'source':'Open-Meteo geocoding with ISO countryCode','elevationSource':'geocoder' if not bad_elev and c.get('elevation') is not None else 'Open-Meteo elevation API'}
...
def forecast(geo:dict[str,Any],venue:dict[str,Any],kick:dt.datetime)->dict[str,Any]:
    ...
    raw=http_json('https://api.open-meteo.com/v1/forecast?'+q); H=raw.get('hourly') or {}; times=H.get('time') or []
...
def repair_weather(d:dict[str,Any],cache:dict[str,Any])->dict[str,Any]:
    ...
    geo_cache=cache.setdefault('geocode',{}); weather_cache=cache.setdefault('weather',{}); now=dt.datetime.now(dt.timezone.utc)
    for row in (d.get('fixtures') or {}).values():
        kick=parse_dt(row.get('kickoff')); venue=(((row.get('travel') or {}).get('home') or {}).get('currentVenue') or {}); city,country=str(venue.get('city') or ''),str(venue.get('country') or '')
        if not kick or not city:continue
        stats['checked']+=1; ck=f'{city}|{country}'; iso=expected_iso(city,country); old=geo_cache.get(ck); current_weather=row.get('weatherVenue') or {}
        if geocode_ok(old,iso):stats['unchanged']+=1;continue
        try:
            g=robust_geocode(city,country)
            if not g or not geocode_ok(g,iso):raise RuntimeError(f'no valid country-consistent geocode for {city}|{country}|{iso}')
            ...
            if kick>=now-dt.timedelta(minutes=30):
                ...
                wk=f"{float(g['lat']):.4f}|{float(g['lon']):.4f}|{kick.isoformat()[:13]}";weather_cache[wk]={'cachedAt':time.time(),'data':w}
            else:
            ...
            stats['repairFailed']+=1;current_weather['contextQualityRepairError']=str(exc)[:180];row['weatherVenue']=current_weather
    return stats
